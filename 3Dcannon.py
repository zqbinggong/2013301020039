...
y_t = ea.enterbox('please enter y-component of the target:',title,image='2.gif')
z_t = ea.enterbox('please enter z-component of the target:',title,image='2.gif')
h=int(h)
...
class CannonShell:
    ...
    def getFactor( self,currentState):
        ...
        factor = 0
        try:
            factor = h*(1 - a * currentState.y / 288.15)**alpha+(1-h)*math.exp(-currentState.y/10000.0)
        except ValueError, e:
            pass
        return factor
...
def updatePositon(objects, dt):
    ...
    def getFactor( object):
        ...
        factor = 0
        try:
            factor = h*(1 - a * object.pos.y  / 288.15)**alpha+(1-h)*math.exp(-object.pos.y /10000.0)
        except ValueError, e:
            pass
        return factor
...
shell = sphere(pos=(0,0,0), radius=2000, color=color.red)
shell.trail = curve(color=color.yellow)
battery = box(pos=(0,-1.1,0), size=(10,0.2,10), color=color.red)
targetPlane = box(pos=tuple(target), size=(20,0.2,20), color=color.red)
axis_x = arrow(pos=(0,0,0), axis=(50000,0,0), shaftwidth=0.01, color=color.blue)
axis_y = arrow(pos=(0,0,0), axis=(0,50000,0), shaftwidth=0.01, color=color.blue)
axis_z = arrow(pos=(0,0,0), axis=(0,0,50000), shaftwidth=0.01, color=color.blue)
